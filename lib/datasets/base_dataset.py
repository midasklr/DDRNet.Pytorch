...
class BaseDataset(data.Dataset):
    ...
    def random_RotateAndReszie(self, img, label):

        if not config.TRAIN.RANDOM_ROTATE:
            return img, label
        if random.random() < 0.5:
            return img, label
        h, w = img.shape[:2]
        (cX, cY) = (w // 2, h // 2)
        angle = random.randint(0,config.TRAIN.RANDOM_ROTATE_ANGLE)
    ...
    def random_RotateAndCrop(self, img, label):
        ...
        label = label[starty:endy, startx:endx]
    ...
    def random_hue(self, img):
        ...
        if not config.TRAIN.HUE:
            return img
        if random.random() < 0.5:
            return img
        hue_value = config.TRAIN.RANDOM_HUE_VALUE
        factor = np.random.uniform(hue_value[0], hue_value[1])
        img = img.astype(np.float32)
        img[...,0] = np.mod(img[...,0] + factor * 180, 180)
        img = np.around(img)
        img = np.clip(img, 0, 255).astype(np.uint8)
        return img
    ...
    def random_saturation(self, img):
        ...
        if not config.TRAIN.SATURATION:
            return img
        if random.random() < 0.5:
            return img
        sta_factor = config.TRAIN.RANDOM_SAT_VALUE
       # print("self.sta_value:",sta_factor)
        factor = np.random.uniform(sta_factor[0], sta_factor[1])
        img = img.astype(np.float32)
    ...
    def random_contrast(self, img):
        ...
        if not config.TRAIN.CONTRAST:
            return img
        if random.random() < 0.5:
            return img
        contrast_factor = config.TRAIN.RANDOM_CONTRAST_VALUE
        factor = np.random.uniform(contrast_factor[0], contrast_factor[1])
        img = img.astype(np.float32)
        mean = img.mean(axis=0).mean(axis=0)
        img = self._clip((img - mean) * factor + mean)
        return img
    ...
    def gen_sample(self, image, label,
        ...
        image, label = self.random_RotateAndCrop(image, label)
