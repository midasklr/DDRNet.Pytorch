...
class FullModel(nn.Module):
  ...
  def pixel_acc(self, pred, label):
    # print('pre:',pred.shape, label.shape)
    if pred.shape[2] != label.shape[1] and pred.shape[3] != label.shape[2]:
        pred = F.interpolate(pred, (label.shape[1:]), mode="nearest")
  ...
  def forward(self, inputs, labels, *args, **kwargs):
    ...
    # print("output:",len(outputs), outputs[0].size(),outputs[1].size())
...
class Map16(object):
    def __init__(self, vedioCap, visualpoint=True):
        self.names = ("background", "floor", "bed", "cabinet,wardrobe,bookcase,shelf",
                ...
                "rug", "railing", "column", "refrigerator", "stairs,stairway,step", "escalator", "wall","c","b","a",
                "dog", "plant")
